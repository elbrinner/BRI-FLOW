...
(function(){
  ...
  // create source input + picker
    ...
    srcInput.placeholder = 'context.items'; srcInput.style.flex = '1';
  ...
  // note: filter/sort moved to advanced section
    ...
    function buildRules(st){
      const rules = [
        ...
        {kind:'warning', when: !!st.source && !/context\./.test(st.source) && !/\{\{/.test(st.source), msg:'sourcelist no parece usar context.* ni variables.'},
