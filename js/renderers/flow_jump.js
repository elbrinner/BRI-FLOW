  ...
  function renderFlowJump(node, container) {
    ...
    goBtn.addEventListener('click', () => {
      try {
        ...
        let fid = flowSel.value || activeFlowId || '';
        ...
        if (typeof window.App?.selectNode === 'function' && nid) {
          window.App.selectNode(nid);
          // Optional: ensure visible if same flow to avoid "lost" feeling without reset
          if (fid === activeFlowId && window.App?.ensureNodeVisible) {
            // Ensure App.ensureNodeVisible is exposed or use internal helper if available? 
            // main.js exposes selectNode but maybe not ensureNodeVisible directly on App.
            // selectNode usually handles some visibility or properties panel show.
            // Let's rely on selectNode for now.
          }
        }
    ...
    goRetBtn.addEventListener('click', () => {
      try {
        const currentFlowId = window.AppProject?.active_flow_id || window.App?.state?.meta?.flow_id || '';
        let fid = rFlowSel.value || currentFlowId || '';
        let nid = rNodeSel.value || '';
        if (!nid) {
          // (next del FlowJump) → ir al flujo actual y seleccionar el next del propio FlowJump
          if (!rFlowSel.value) fid = currentFlowId; // if returning to call stack implicit, we might not know target. 
          // But here UI assumes explicit selection or current.
          nid = window.App?.state?.nodes?.[node.id]?.next?.node_id || '';
        }
        if (!fid) return alert('Selecciona un flujo de retorno');

        if (fid !== currentFlowId) {
          if (window.AppProject?.flows?.[fid]) {
            const f = window.AppProject.flows[fid];
            if (window.App && typeof window.App.importJson === 'function') window.App.importJson({ flow_id: fid, meta: f.meta, nodes: f.nodes });
            window.AppProject.active_flow_id = fid;
