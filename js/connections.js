...
(function(){
  ...
  function resolveTargetRef(t) {
    ...
    if (typeof t === 'object' && t.node_id) return t.node_id;
    return null;
  }
  ...
  function refreshConnections(state) {
    ...
    for (const id in state.nodes) {
      ...
      if (!((node.type === 'button' || node.type === 'multi_button') && node.mode !== 'dynamic')) {
        pushSpec(id, resolveTargetRef(node.next), null);
      }
      ...
      if (node.type === 'choice' && (node.mode === 'switch')) {
        if (Array.isArray(node.cases)) {
          for (let k = 0; k < node.cases.length; k++) {
            ...
            const tgt = resolveTargetRef(c.target);
        ...
        if (node.default_target) {
          const dt = resolveTargetRef(node.default_target);
      ...
      if (Array.isArray(node.connections)) {
        for (let i = 0; i < node.connections.length; i++) {
          ...
          pushSpec(id, resolveTargetRef(c), (c && c.label) ? c.label : null);
        }
      }
      ...
      if (Array.isArray(node.options)) {
        ...
        if (!isBtnLikeDynamic) {
          for (let j = 0; j < node.options.length; j++) {
            ...
            const tgt = resolveTargetRef(opt.next || opt.target);
            let lbl = opt.label || '';
      ...
      // condition
      pushSpec(id, resolveTargetRef(node.true_target), 'TRUE');
      pushSpec(id, resolveTargetRef(node.false_target), 'FALSE');
      // loop/foreach
      pushSpec(id, resolveTargetRef(node.body_start), 'BODY');
      pushSpec(id, resolveTargetRef(node.after_loop), 'AFTER');
      if (node.loop_body) pushSpec(id, resolveTargetRef(node.loop_body), '🔁 Loop', { isLoopBody: true });
      // set_goto return
      if (node.type === 'set_goto' && node.target) pushSpec(id, resolveTargetRef(node.target), 'return', { isGotoReturn: true });
      // flow_jump y su return_target
      if (node.type === 'flow_jump') {
        pushSpec(id, resolveTargetRef(node.target), 'Jump');
        if (node.return_target) pushSpec(id, resolveTargetRef(node.return_target), 'return', { isGotoReturn: true });
      }
    }
