(function () {
  ...
  window.Simulador.nodes.processResponse = function (node, state, flow, appendChatMessage, showTyping, gotoNext, log, evaluate, maybeAppendDiff, renderPreview, renderVariables, step, stepDelay, fastMode, running, stepTimeout, done) {
    ...
    function handleOptions(node, state, flow, appendChatMessage, gotoNext, log, evaluate, maybeAppendDiff, renderPreview, renderVariables, step, stepDelay, fastMode, running, stepTimeout) {
      ...
      // Schedule next step to continue execution
      if (typeof step === 'function') {
        setTimeout(() => { try { step(); } catch (_e) { } }, stepDelay || 600);
      }
    }
  };
  ...
  // Register the response node handler (wait for registry to be available)
  function tryRegister() {
    if (window.Simulador && window.Simulador.nodes && typeof window.Simulador.nodes.register === 'function') {
      window.Simulador.nodes.register('response', function (node, state, flow, nodeId, ctx) {
        console.log('[processResponse] Handler called for node:', nodeId);
        // Adapter: convert new context-based signature to old parameter-based signature
        const dummyShowTyping = function (callback) { if (callback) callback(); }; // No-op
        window.Simulador.nodes.processResponse(
          node, state, flow,
          ...
          ctx.log,
          ctx.evaluate,
          null, // maybeAppendDiff (deprecated)
          ctx.renderPreview,
          ctx.renderVariables,
          ctx.step,
          ctx.stepDelay,
      ...
      console.log('[processResponse.js] response handler registered');
    } else {
